#include "ndt_localization/ndt.h"

NDTLocalizer::NDTLocalizer(const rclcpp::NodeOptions &options = rclcpp::NodeOptions()) : Node("ndt_localizer", options)
{
    key_value_stdmap_["state"] = "Initializing";
    init_params();

    // publisher
    sensor_aligned_pose_pub_ = this->create_publisher<sensor_msgs::msg::PointCloud2>("sensor_aligned_pose", 10);
    ndt_pose_pub_ = this->create_publisher<geometry_msgs::msg::PoseStamped>("ndt_pose", 10);
    exe_time_pub_ = this->create_publisher<std_msgs::msg::Float32>("exe_time", 10);
    ...
    // subscriber, the param of the callback function is not reference
    initial_pose_sub_ = this->create_subscription<geometry_msgs::msg::PoseWithCovarianceStamped>(
                            "initial_pose", 10, std::bind(&NDTLocalizer::callback_init_pose, this, std::placeholders::_1));
    ...
    sensor_points_sub_ = this->create_subscription<sensor_msgs::msg::PointCloud2>(
                            "/livox/lidar/pointcloud", 1, std::bind(&NDTLocalizer::callback_pointcloud, this, std::placeholders::_1));
...
void NDTLocalizer::timer_diagnostic()
{
    ...
    while(rclcpp::ok())
    {
        ...
        diag_msg.header.stamp = rclcpp::Clock().now();
...
void NDTLocalizer::callback_pointsmap(const sensor_msgs::msg::PointCloud2::ConstPtr pointcloud2_msg_ptr)
{
    ...
    RCLCPP_INFO(this->get_logger(), "get pointmap!!!");
    // swap
    ndt_map_mtx_.lock();
    ndt_ = ndt_new;
    ndt_map_mtx_.unlock();
...
void NDTLocalizer::callback_pointcloud(const sensor_msgs::msg::PointCloud2::ConstPtr sensor_points_sensorTF_msg_ptr)
{
    ...
    // RCLCPP_INFO(this->get_logger(), "get map to base transform!!!");
    ...
    *sensor_points_baselinkTF_ptr = *sensor_points_sensorTF_ptr;
    ...
    // set input point cloud setInputTarget
    ndt_.setInputSource(sensor_points_baselinkTF_ptr);

    RCLCPP_INFO(this->get_logger(), "input source pcd!!!");
    if (ndt_.getInputTarget() == nullptr) 
    {
        RCLCPP_WARN(this->get_logger(), "Map cloud is not set yet, waiting for it...");
        return;
    }
    // align
    ...
    // update diagnostic
    key_value_stdmap_["seq"] = std::to_string(sensor_points_sensorTF_msg_ptr->header.stamp.sec);
...
void NDTLocalizer::init_params()
{
    ...
    this->declare_parameter("trans_epsilon", trans_epsilon);
    this->declare_parameter("step_size", step_size);
    this->declare_parameter("resolution", resolution);
    this->declare_parameter("max_iterations", max_iterations);
...
void NDTLocalizer::callback_init_pose(
  const geometry_msgs::msg::PoseWithCovarianceStamped::ConstPtr initial_pose_msg_ptr)
{
  ...
  const geometry_msgs::msg::TransformStamped::Ptr & transform_stamped_ptr)
{
    if (target_frame == source_frame) 
    {
        transform_stamped_ptr->header.stamp = rclcpp::Clock().now();
        transform_stamped_ptr->header.frame_id = target_frame;
        transform_stamped_ptr->child_frame_id = source_frame;
        transform_stamped_ptr->transform.translation.x = 0.0;
        transform_stamped_ptr->transform.translation.y = 0.0;
...
int main(int argc, char ** argv)
{
    ...
    auto node = std::make_shared<NDTLocalizer>(options);
    rclcpp::spin(node);
    rclcpp::shutdown();
    return 0;
}
